package com.luml.sence.uniquecode.snowflake;
 ...
 * 这里的的开始时间截，一般是我们的id生成器开始使用的时间，由我们程序来指定的
 （如下下面程序SnowflakeIdUtil类的twepoch属性）。
 * 41位的时间截，可以使用69年，年T = (1L << 41) / (1000L * 60 * 60 * 24 * 365) = 69
 * 10位的数据机器位，可以部署在1024个节点，包括5位datacenterId和5位workerId，可以合在一起使用，
 也可以分开使用
...
public class SnowflakeUtil {
    ...
    private SnowflakeUtil() {
        ...
        if (datacenterId > maxDatacenterId || datacenterId < 0) {
            throw new IllegalArgumentException( String.format("datacenterId（０～%d）设置错误",
                    maxDatacenterId));
        }
